...
abstract class EntityNavigator{
	...
	abstract public function makeRandomGoal() : Position;

	abstract public function getDefaultPathFinder() : PathFinder;

	public function __construct(protected LivingBase $holder){}

	public function update() : void{
		$pos = $this->holder->getLocation();
		$holder = $this->holder;
		$target = $holder->getTargetEntity();
		if($target === null || !$holder->canInteractWithTarget($target, $pos->distanceSquared($target->getPosition()))){
			$near = PHP_INT_MAX;
			$target = null;
